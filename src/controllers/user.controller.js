...
const changeCurrentPassword = asyncHandler(async (req, res) => {
   ...
   // agr banda pass change kr paa rha ha mtlb wo logged in hai
   // humne auth middleware se req me user ka access de diya tha
   ...
   return res
      .status(200)
      .json(new ApiResponse(200, {}, "Password saved sucessfully"))
})
...
const updateAccountDetails = asyncHandler(async (req, res) => {
   ...
   const user = await User.findByIdAndUpdate(
      ...
      {
         $set: {
            fullname,  // this command is equal to fullname:fullname
            email: email
         }
      },
      {
         // new true krne se jo informate update hoti ha wo return ho jati h 
         new: true
      }
   ).select("-password")
...
const updateUserCoverImage = asyncHandler(async (req, res) => {
   ...
   const coverImageLocalPath = req.files?.coverImage[0]?.path; 
   // agr sirf ek hi file h to niche wali bhi chal jayegi upar wali line ki jagah
